...
function TestDriver() {
     ...
     *
     */
    TestDriver.prototype.registerDevicePlugin = function (plugin, path) {
        if (!plugin && !path) {
            throw 'API has changed for TestDriver.prototype.registerDevicePlugin. Provide (plugin, path)';
        }
        ...
        this.plugins[plugin + "/" + devicePlugin.plugin] = devicePlugin;
    };
     ...
     *
     */
    TestDriver.prototype.publishActorOperationalStateChange = function (device, actor, state) {
        this.logInfo("Publishing Operational State Change for Actor [" + actor.id + "] in Device [" + device.id + "]: ", state);

        for (var n in this.listeners) {
            if (this.listeners[n].publishActorOperationalStateChange) {
                this.listeners[n].publishActorOperationalStateChange(device, actor, state);
            }
        }
    };
     ...
     *
     */
    TestDriver.prototype.publishSensorStateChange = function (device, sensor, state) {
        this.logInfo("Publishing State Change for Sensor [" + sensor.id + "] in Device [" + device.id + "]: ", state);

        for (var n in this.listeners) {
            if (this.listeners[n].publishSensorStateChange) {
                this.listeners[n].publishSensorStateChange(device, sensor, state);
            }
        }
    };
